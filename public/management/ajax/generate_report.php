...
// Check if user is logged in - session is already started by config.php
...
try {
    ...
    switch($format) {
        case 'pdf':
            ...
            // Suppress TCPDF warnings that might be output
            ...
            try {
                ...
                $pdf->SetTitle("Report - " . ucfirst(str_replace('_', ' ', $report_type)));
                ...
                // Add report content
                $html = '<h1 style="text-align:center;">MTECH UGANDA</h1>';
                $html .= '<h2 style="text-align:center;">' . ucfirst(str_replace('_', ' ', $report_type)) . ' Report</h2>';
                $html .= '<p><strong>Period:</strong> ' . date('F d, Y', strtotime($start_date)) . ' to ' . date('F d, Y', strtotime($end_date)) . '</p>';
                $html .= '<p><strong>Generated on:</strong> ' . date('F d, Y H:i:s') . '</p>';
                $html .= '<p><strong>Generated by:</strong> ' . ($_SESSION['full_name'] ?? $_SESSION['username']) . '</p>';
                ...
                if (!empty($data)) {
                    ...
                    foreach (array_keys($data[0]) as $header) {
                        $html .= "<th style='background-color:#e0e0e0; font-weight:bold;'>" . ucwords(str_replace('_', ' ', $header)) . "</th>";
                    }
                    $html .= "</tr></thead><tbody>";
                ...
                // Add footer info
                $html .= "<p style='text-align:center; font-size:9pt; margin-top:15px;'>Copyright © " . date('Y') . " MTECH UGANDA. All rights reserved.</p>";
                ...
                // Clear any output buffers and headers
                while (ob_get_level()) {
                    ob_end_clean();
                }
        ...
        case 'excel':
            ...
            header('Content-Disposition: attachment;filename="MTECH_UGANDA_' . $report_type . '_report_' . date('Y-m-d') . '.xls"');
            
            echo "<!DOCTYPE html>
                <html>
                <head>
                    <meta charset='UTF-8'>
                    <title>MTECH UGANDA Report</title>
                    <style>
                ...
                <body>
                    <h1>MTECH UGANDA</h1>
                    <h2>" . ucfirst(str_replace('_', ' ', $report_type)) . " Report</h2>
            ...
            if (!empty($data)) {
                ...
                echo "<tr><td colspan='10' style='text-align:center;'>No data available for the selected criteria</td></tr>";
            }
